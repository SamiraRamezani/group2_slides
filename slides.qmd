...
author: Samira Ramezani
format: revealjs
---

# What is Async/Await?

This report explores the use of asynchronous programming in JavaScript, especially comparing `async/await` with `Promise chaining`. The discussion is based on a project that interacts with a REST API and uses JavaScript classes to model and manipulate student data.

In the project, all network operations are handled asynchronously using `async/await`.
...
## Why Prefer Async/Await?

- **Readability:** Code looks cleaner and easier to follow  
- **Error Handling:** Try/catch makes catching errors straightforward  
- **Maintainability:** Easier to write and refactor complex async flows
